...
final class ProtocolClient extends WebSocketListener {
    ...
    static final String VERSION="3.0.0-alpha.4-android";
    ...
    private JSONArray capabilities(){JSONArray out=new JSONArray();if(state.tree()==null)return out;for(int i=0;i<readCaps.length();i++)out.put(readCaps.optString(i));if("write".equals(state.mode()))out.put("file_edit").put("directory_create").put("workspace_clear").put("code_edit");return out;}
    ...
    private JSONObject nativeShareProfile()throws Exception{JSONObject resources=new JSONObject().put("advertise",state.resourceAdvertise());if(state.resourceAdvertise())resources.put("inventory",deviceResources());return new JSONObject().put("visibility",state.visibility()).put("resources",resources).put("compute",new JSONObject().put("remote_requested",state.remoteCompute()).put("runtime","cluster").put("available",false));}
    private void connect(){
        if(stopped.get()||!connecting.compareAndSet(false,true))return;
        if(state.tree()==null&&!state.resourceAdvertise()&&!state.remoteCompute()){connecting.set(false);listener.onState("Choose a workspace or enable a native share first");return;}
        workspace=null;
