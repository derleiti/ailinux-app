...
const aicoderRuntime = require('./aicoder_runtime');

const APP_NAME = 'AILinux App';
...
function registerHelperIpc() {
  ipcMain.handle('ailinux-app:aicoder-status', (event) => { assertTrustedIpc(event); return aicoderRuntime.status(); });
  ipcMain.handle('ailinux-app:aicoder-launch', (event) => { assertTrustedIpc(event); return aicoderRuntime.launch(['gui']); });
  ipcMain.handle('ailinux-app:aicoder-stop', (event) => { assertTrustedIpc(event); return aicoderRuntime.stop(); });
...
function rebuildTrayMenu() {
  ...
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Open AILinux App', click: showWindow },
    { label: 'Open AICoder', enabled: aicoderRuntime.status().available, click: () => aicoderRuntime.launch(['gui']) },
    ...
    { type: 'separator' },
    { label: 'Quit AILinux App', click: () => { quitting = true; aicoderRuntime.stop(); app.quit(); } },
  ]));
}
