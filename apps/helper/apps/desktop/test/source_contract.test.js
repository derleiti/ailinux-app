...
test('android folder changes rebind the live share while fresh pairing stays explicit', () => {
  const activity = fs.readFileSync(path.join(__dirname, '..', '..', 'android', 'app', 'src', 'main', 'java', 'me', 'ailinux', 'workspace', 'WorkspaceActivity.java'), 'utf8');
...
test('android Termux remains device-local and is never advertised as a public share tool', () => {
  ...
  const activity = fs.readFileSync(path.join(root, 'java', 'me', 'ailinux', 'workspace', 'WorkspaceActivity.java'), 'utf8');
...
test('android share profile supports native-only resource advertisement and remote compute preference', () => {
  ...
  const activity = fs.readFileSync(path.join(root, 'WorkspaceActivity.java'), 'utf8');
