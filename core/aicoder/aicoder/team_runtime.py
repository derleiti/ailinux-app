...
Preferred tools for this stage: `file_tree`, `file_read`, `code_tree`, `code_search`, `code_read`, `git`, `shell`, `binary_exec`, `task_runner`, `lint`, and `test`, plus any repository-specific observational tool required to derive deterministic verification. All authenticated tools remain available; this stage plans/verifies and does not persist implementation mutations. For read-only inspection, prefer typed read tools; never spawn Python, shell, or binary_exec merely to read, tail, parse, filter, or summarize text/JSON/JSONL. Use binary_exec only when actual executable behavior is required."""
...
a concise implementation and verification summary. Preferred tools for this stage: `file_tree`, `file_read`, `code_tree`, `code_search`, `code_read`, `file_edit`, `directory_create`, `shell`, `binary_exec`, `task_runner`, `git`, `lint`, and `test`; use additional authenticated tools whenever they materially help complete or verify the contract. For read-only inspection, prefer typed read tools; never spawn Python, shell, or binary_exec merely to read, tail, parse, filter, or summarize text/JSON/JSONL. Use binary_exec only when actual executable behavior is required. Use `.aicoder-team/stageoff.json` as authoritative run memory. If no repository change is genuinely justified, use exactly
...
Preferred tools for this stage: `.aicoder-team/stageoff.json` via `file_read`, candidate evidence via `file_tree`/`file_read`, then `code_search`, `code_read`, `file_edit`, `directory_create`, `shell`, `binary_exec`, `task_runner`, `git`, `lint`, and `test`. All authenticated tools remain available; integrate only evidence-backed changes and re-verify the complete merged workspace. For read-only inspection, prefer typed read tools; never spawn Python, shell, or binary_exec merely to read, tail, parse, filter, or summarize text/JSON/JSONL. Use binary_exec only when actual executable behavior is required."""
